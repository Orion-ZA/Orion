...
describe('TrailsPanel', () => {
  ...
  test('distanceAway sorting falls back when no userLocation', async () => {
    const trails = [makeTrail({ id: '1', name: 'B' }), makeTrail({ id: '2', name: 'A' })];
  ...
  test('scrolling list unselects trail after debounce', async () => {
    ...
    render(
      <TrailsPanel
        {...baseProps({ filteredTrails: [trail], selectedTrail: trail, setSelectedTrail })}
      />
    );
  ...
  test('author display handles sample, unknown, raw id and fetched name', async () => {
    ...
    mockGetDoc.mockResolvedValueOnce({
      exists: () => true,
      data: () => ({ profileInfo: { displayName: 'John Doe' } }),
    });
    const trails = [
  ...
  test('edit button appears only for trails created by current user and fires onEditTrail', async () => {
    ...
    const editBtns = screen
      .getAllByRole('button')
      .filter(b => b.title && b.title.startsWith('Edit '));
  ...
  test('filters out trails with invalid coordinates', () => {
    const trails = [
      ...
      makeTrail({
        id: 'invalid-lat',
        name: 'Invalid Lat Trail',
        latitude: 'invalid',
        longitude: 18.4,
      }),
      makeTrail({
        id: 'invalid-lng',
        name: 'Invalid Lng Trail',
        latitude: -33.9,
        longitude: 'invalid',
      }),
      makeTrail({
        id: 'missing-lat',
        name: 'Missing Lat Trail',
        latitude: undefined,
        longitude: 18.4,
      }),
      makeTrail({
        id: 'missing-lng',
        name: 'Missing Lng Trail',
        latitude: -33.9,
        longitude: undefined,
      }),
  ...
  test('handles trails with undefined/null coordinates gracefully', () => {
    const trails = [
      ...
      makeTrail({
        id: 'undefined-lat',
        name: 'Undefined Lat Trail',
        latitude: undefined,
        longitude: 18.4,
      }),
      makeTrail({
        id: 'undefined-lng',
        name: 'Undefined Lng Trail',
        latitude: -33.9,
        longitude: undefined,
      }),
    ];
  ...
  test('handles missing trail properties in sorting', () => {
    const trails = [
      ...
      }), // Missing distance, difficulty, elevation
      makeTrail({
        id: 'trail2',
        name: 'Trail 2',
        distance: '5',
