...
export const useTrailAlerts = () => {
  ...
  const fetchMultipleTrailAlerts = async trailIds => {
    ...
    try {
      ...
      console.log(
        `Found ${Object.keys(cachedResults).length} cached, fetching ${uncachedTrailIds.length} from database`
      );
      ...
      if (uncachedTrailIds.length > 0) {
        ...
        console.log(
          `Making ${batches.length} database queries for ${uncachedTrailIds.length} trails`
        );
