...
describe('TabSection', () => {
  ...
  describe('Alerts Tab', () => {
    ...
    it('should show no alerts message when alerts array is empty', () => {
      ...
      expect(screen.getByText('No recent alerts for this trail. Check back later or add an alert if you notice something important.')).toBeInTheDocument();
    });
    ...
    it('should render alert timer with only minutes and seconds when no hours', () => {
      ...
      render(<TabSection {...defaultProps} activeTab="alerts" alerts={alertsWithNearFutureExpiry} />);
