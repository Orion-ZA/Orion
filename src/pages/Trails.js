...
export default function TrailsPage() {
  ...
  const [submissionDrawingState, setSubmissionDrawingState] = useState({
    isDrawing: false,
    addRoutePoint: null,
  });
  ...
  // Use trails hook
  const { trails, isLoadingTrails, filters, handleFilterChange, filteredTrails } = useTrails(
    trailsUserLocation,
    currentUserId
  );
    ...
    async query => {
      ...
      const matchingTrail =
        trails && trails.length > 0
          ? trails.find(
              trail => trail.name && trail.name.toLowerCase().includes(query.toLowerCase())
            )
          : null;
  ...
  const handleMapClickForSubmission = useCallback(
    async evt => {
  ...
  const handleTrailSubmission = async trailData => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        await updateDoc(userRef, { submittedTrails: arrayUnion(trailRef) });
  ...
  const handleTrailUpdate = async trailData => {
    ...
    try {
      ...
      if (
        updateData.gpsRoute &&
        Array.isArray(updateData.gpsRoute) &&
        updateData.gpsRoute.length > 0
      ) {
