  ...
  cors(req, res, async () => {
    ...
    try {
      ...
      if (!validDifficulties.includes(trailData.difficulty))
        return res.status(400).json({ error: 'Difficulty must be one of: Easy, Moderate, Hard' });
      ...
      if (typeof trailData.elevationGain !== 'number' || trailData.elevationGain < 0)
        return res.status(400).json({ error: 'Elevation gain must be a non-negative number' });
      ...
      const trailDocument = {
        ...
        gpsRoute:
          trailData.gpsRoute && Array.isArray(trailData.gpsRoute)
      ...
      if (req.headers.authorization) {
        try {
          ...
          console.warn(
            'Invalid auth token, creating trail without user reference:',
            authError.message
          );
        }
      }
      ...
      validFields.forEach(field => {
        if (updates[field] !== undefined) {
          if (field === 'gpsRoute' && Array.isArray(updates[field])) {
            dataToUpdate.gpsRoute = updates[field].map(
...
exports.addTrailReview = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    ...
    if (!trailId || !review)
      return res.status(400).json({ error: 'trailId and review are required.' });
