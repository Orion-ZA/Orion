...
export const SearchProvider = ({ children }) => {
  ...
  const getSuggestions = useCallback(
    query => {
      ...
      const realTrailMatches = trailsData
        .filter(trail => trail.name && trail.name.toLowerCase().includes(query.toLowerCase()))
        .sort((a, b) => {
          ...
          if (mapboxToken) {
            ...
            if (geocodingResponse.ok) {
              ...
              geocodingSuggestions =
                geocodingResults.features?.map(feature => {
                  ...
                  const isTrailRelated =
                    feature.properties?.category?.toLowerCase().includes('trail') ||
  ...
  const getLocationNameFromCoordinates = useCallback(
    async coordinates => {
      try {
