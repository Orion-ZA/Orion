  ...
  return function MockTrailDetailHeader({ onBack, onShowOnMap, onShare, onReport }) {
    return (
      <div data-testid="trail-detail-header">
        ...
        <button onClick={onShowOnMap} data-testid="show-on-map-button">Show on Map</button>
        <button onClick={onShare} data-testid="share-button">Share</button>
        <button onClick={onReport} data-testid="report-button">Report</button>
      </div>
    );
  };
});
  ...
  return function MockTrailDetailActions({ user, trail, userSaved, onTrailAction }) {
    return (
      <div data-testid="trail-detail-actions">
        <button onClick={() => onTrailAction('favourites', trail.id)} data-testid="favourite-button">
          Favourite
        </button>
      </div>
    );
  ...
  return function MockTrailImageGallery({ images, currentImageIndex, onImageChange, onGoToImage }) {
    return (
      <div data-testid="trail-image-gallery">
        ...
        <button onClick={() => onImageChange(1)} data-testid="next-image-button">Next</button>
        <button onClick={() => onGoToImage(0)} data-testid="go-to-image-button">Go to Image</button>
      </div>
    );
  };
});
  ...
  return function MockTrailInfo({ trail, authorName, onDirections, estimateDuration }) {
    return (
      <div data-testid="trail-info">
        <h1>{trail.name}</h1>
        <p>By {authorName}</p>
        <button onClick={onDirections} data-testid="directions-button">Get Directions</button>
  ...
  return function MockTabSection({ 
    ...
    return (
      <div data-testid="tab-section">
        <button onClick={() => setActiveTab('reviews')} data-testid="reviews-tab">Reviews</button>
        <button onClick={() => setActiveTab('media')} data-testid="media-tab">Media</button>
        <button onClick={() => setActiveTab('alerts')} data-testid="alerts-tab">Alerts</button>
        <button onClick={() => onOpenContributionModal('review')} data-testid="add-review-button">Add Review</button>
        <button onClick={() => onReport('review', 'review-1')} data-testid="report-review-button">Report Review</button>
      </div>
    );
  };
});
  ...
  return function MockSuccessPopup({ isVisible, message, onClose }) {
    ...
    return (
      <div data-testid="success-popup">
        <p>{message}</p>
        <button onClick={onClose} data-testid="close-success-button">Close</button>
      </div>
    );
  };
});
...
describe('TrailDetail', () => {
  ...
  describe('Component Props Passing', () => {
    ...
    it('passes correct props to TrailImageGallery', () => {
      ...
      expect(screen.getByAltText('Trail image')).toHaveAttribute('src', 'https://example.com/image1.jpg');
    });
