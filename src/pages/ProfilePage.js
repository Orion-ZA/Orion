    ...
    const unsubscribe = auth.onAuthStateChanged(async u => {
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          const [wishlist, favourites, completed, submittedTrails] = await Promise.all([
  ...
  return (
    <div className='profile-page'>
      ...
      <div className='profile-card'>
        {user?.photoURL ? (
          <img src={user.photoURL} alt='User Avatar' className='profile-avatar-large' />
        ) : (
      ...
      <div className='stats-grid'>
        <div className='stat-card'>
          <SubmittedIcon />
          <h3>{trailsData.submittedTrails.length}</h3>
          <p>Submitted</p>
        </div>
        <div className='stat-card'>
          <CompletedIcon />
          <h3>{trailsData.completed.length}</h3>
          <p>Completed</p>
        </div>
        <div className='stat-card'>
          <FavouritesIcon />
          <h3>{trailsData.favourites.length}</h3>
          <p>Favourites</p>
        </div>
        <div className='stat-card'>
          <WishlistIcon />
          <h3>{trailsData.wishlist.length}</h3>
          <p>Wishlist</p>
        </div>
      </div>
