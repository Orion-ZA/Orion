      ...
      <div className='trail-card-header'>
        <div className='trail-card-info-main'>
          ...
          <div className='trail-card-meta'>
            ...
            <div className='trail-card-meta-item'>
              <Target
                className='trail-card-meta-icon'
            ...
            <div className='trail-card-meta-item'>
              <span
                className={`trail-card-status-badge trail-card-status-${String(trail.status || 'open')}`}
              >
                  ...
                  <div key={review.id} className='trail-card-review-item'>
                    <div className='trail-card-review-header'>
                      <div className='trail-card-review-rating'>
                        ...
                        <span className='trail-card-rating-text'>
                          ({typeof review.rating === 'number' ? review.rating : 0}/5)
                        </span>
                      </div>
                      <button
                        onClick={() =>
                          onDeleteReview(
                            review.id,
                            String(trail.id),
                            String(trail.name || 'Unnamed Trail')
                          )
                        }
                    ...
                    <p
                      className={`trail-card-review-comment ${!(review.comment || review.message) ? 'no-comment' : ''}`}
                    >
                      {review.comment || review.message
                        ? `"${String(review.comment || review.message)}"`
                        : 'No comment provided'}
                    </p>
                    <div className='trail-card-review-meta'>
                      <span
                        className='trail-card-review-user'
                        title={String(review.userId || 'Unknown')}
                      >
            ...
            <div className='trail-card-scrollable-container'>
              {loadingStates.alerts?.[String(trail.id)] ? (
                ...
                alertsForTrail.map(alert => (
                  <div
                    key={alert.id}
                    className={`trail-card-alert-item ${alert.isActive ? 'active' : 'inactive'}`}
                  >
                    <div className='trail-card-alert-header'>
                      <div className='trail-card-alert-type'>
                        ...
                        <span
                          className={`trail-card-alert-status ${alert.isActive ? 'active' : 'inactive'}`}
                        >
                    ...
                    <p className='trail-card-alert-message'>
                      {String(alert.message || alert.comment || 'No message')}
                    </p>
