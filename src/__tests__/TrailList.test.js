...
const mockTrails = [
  ...
  },
  {
    ...
    distance: 8.0,
    elevationGain: 600,
    location: { latitude: -26.15, longitude: 27.95 },
  },
];
...
describe('TrailList', () => {
  ...
  describe('Trail Selection', () => {
    ...
    it('highlights selected trail', async () => {
      ...
      await waitFor(() => {
        const trailElement = screen
          .getByText('Test Trail 1')
          .closest('div[style*="background-color"]');
  ...
  describe('API Integration', () => {
    ...
    it('does not fetch reviews for trails with existing averageRating', async () => {
      ...
      await waitFor(() => {
        ...
        expect(fetch).not.toHaveBeenCalledWith(expect.stringContaining('trailId=1'));
      });
    });
  ...
  describe('Rating Calculation', () => {
    it('calculates average rating correctly', async () => {
      const reviewsWithRatings = [{ rating: 5 }, { rating: 4 }, { rating: 3 }, { rating: 2 }];
    ...
    it('handles reviews with missing ratings', async () => {
      const reviewsWithMissingRatings = [{ rating: 5 }, { rating: null }, { rating: 3 }];
  ...
  describe('Distance Calculation', () => {
    ...
    it('does not call calculateDistance when user location is not provided', async () => {
      ...
      render(
        <TrailList {...defaultProps} userLocation={null} calculateDistance={calculateDistance} />
      );
