...
const NON_SEMANTIC_PATTERNS = [
  ...
  },
  {
    ...
    suggestion: 'Replace <div class="section"> with <section> or <div class="article"> with <article>'
  },
  {
...
class SemanticHTMLChecker {
    ...
    lines.forEach((line, index) => {
      ...
      if (imgRegex.test(line)) {
        issues.push({
          ...
          suggestion: this.options.fixSuggestions ? 'Add alt="descriptive text" to <img> tag' : undefined,
          severity: 'error'
        });
      }
      ...
      if (inputRegex.test(line) && !line.includes('type="hidden"')) {
        issues.push({
          ...
          suggestion: this.options.fixSuggestions ? 'Add aria-label="descriptive text" or wrap with <label>' : undefined,
          severity: 'warning'
        });
      }
      ...
      if (headingMatch) {
        ...
        if (level > 1 && !this.hasPreviousHeading(level - 1, lines.slice(0, index))) {
          issues.push({
            ...
            suggestion: this.options.fixSuggestions ? 'Ensure proper heading hierarchy (h1 -> h2 -> h3, etc.)' : undefined,
            severity: 'warning'
          });
        }
      }
  ...
  generateReport() {
    ...
    if (this.results.semanticElementsUsed.size > 0) {
      ...
      Array.from(this.results.semanticElementsUsed).sort().forEach(element => {
    ...
    if (this.results.nonSemanticElementsUsed.size > 0) {
      ...
      Array.from(this.results.nonSemanticElementsUsed).sort().forEach(element => {
    ...
    if (this.results.fileStats.size > 0) {
      ...
      sortedFiles.forEach(([filePath, stats]) => {
        ...
        if (totalNonSemantic > 0) {
          ...
          if (totalElements > 0) {
            const semanticRatio = ((totalElements - totalNonSemantic) / totalElements * 100).toFixed(1);
  ...
  generateRecommendations() {
    ...
    const divCount = this.results.issues.filter(issue => 
    ...
    if (divCount > 0) {
      recommendations.push(`Replace ${divCount} non-semantic <div> elements with semantic alternatives (header, nav, main, footer)`);
    }
    
    const buttonCount = this.results.issues.filter(issue => 
      issue.content.includes('onClick') && 
      (issue.content.includes('<div') || issue.content.includes('<span'))
    ).length;
    
    if (buttonCount > 0) {
      recommendations.push(`Replace ${buttonCount} interactive <div>/<span> elements with <button> for better accessibility`);
    }
    ...
    if (headingCount > 0) {
      recommendations.push(`Fix ${headingCount} heading hierarchy issues for better document structure`);
    }
    
    const accessibilityCount = this.results.issues.filter(issue => 
      issue.severity === 'error'
    ).length;
    
    if (accessibilityCount > 0) {
      recommendations.push(`Fix ${accessibilityCount} accessibility issues (missing alt attributes, labels)`);
    }
  ...
  showTopFilesNeedingAttention() {
    ...
    sortedFiles.forEach(([filePath, stats], index) => {
      ...
      console.log(`      <div>: ${stats.divCount} | <span>: ${stats.spanCount} | Total: ${totalNonSemantic}`);
